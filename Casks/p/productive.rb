cask "productive" do
  ...
  url "https://download.productive.io/desktop/electron/Productive-#{version}#{arch}.dmg",
      verified: "download.productive.io/"
  name "Productive"
  desc "All-in-one agency managment system"
  homepage "https://productive.io/"

  livecheck do
    url "https://productive.io/apps/"
    regex(%r{href=.*?/Productive[._-]v?(\d+(?:\.\d+)+)\.dmg}i)
  end
  ...
  zap trash: [
    ...
    "~/Library/HTTPStorages/io.productive",
    "~/Library/Logs/productive",
    "~/Library/Preferences/io.productive.plist",
    "~/Library/Saved Application State/io.productive.savedState",
    "~/Library/Application Support/CrashReporter/Productive_*.plist",
  ]
end
